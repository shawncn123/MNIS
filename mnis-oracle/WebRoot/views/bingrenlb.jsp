...
<script type="text/javascript">
	...
	$(function() {$(document).ready(function() {
		...
		$("#chacw").click(function() {
			...
			$.post(url,args,function(data) {
				if(data != ""){
					...
					<a href="bingreng_yewumokuai?v_key1=' 
						+ d[i].key1 + '&v_key2=' + d[i].key2
					 */
				}else{
		...
		/* $("#saowandai").ondataavailable(function() {
			var val = $("#saowandai").val();
			var url = "chabingren_byWanDai";
			var args = {"wandaixx" : val};
			$.post(url,args,function(data) {
				if(data != ""){
					d = eval("("+ data+ ")");
					var a = "<p>姓名：<strong style='font-size: 20px;'>" + d.br_name + "</strong></p>" + "<p>床位号：<strong style='font-size: 20px;'>" + d.br_chw
		...
		$(function () {  
		  ...
		  /*   $('#information').blur(function(){	    //input失去焦点且有值的时候弹出模态框
				var val = $(this).val();
				if (val.length>0) {
					$('#informationModal').modal('show');
				}
